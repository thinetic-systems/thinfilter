...
def __init__():
    """
    read contents of server dir and put in __all__ list
    """
    ...
    for file_ in __glob__.glob(_ext_dir+"/*.py"):
        ...
        try:
            if __sys__.version_info[0:3] < (2, 5, 0):
                __import__('thinfilter.server.' + _ext_name, globals(), locals(), ['server'] ) 
            else:
                __import__('thinfilter.server.' + _ext_name, fromlist = ['server'] ) 
